...
local function addExp(player, amount)
	...
	if xpVal and levelVal then
		...
		if xpVal.Value >= requiredXP then
			...
			-- == ЗВУК ПОВЫШЕНИЯ УРОВНЯ (НОВОЕ) ==
			-- Сюда потом вставь свой ID, например "rbxassetid://12345678"
			local levelUpSoundId = "rbxassetid://2686079706" 

			local sound = Instance.new("Sound")
			sound.Name = "LevelUpSfx"
			sound.SoundId = levelUpSoundId
			sound.Volume = 1
			sound.Parent = player.Character or player.PlayerGui
			sound:Play()
			game:GetService("Debris"):AddItem(sound, 3) -- Удаляем звук через 3 сек
			-- ===================================

			-- ПАУЗА ИГРЫ И ЗАМОРОЗКА ВСЕХ
			session.isPaused = true 
			toggleZombies(true) 
			togglePlayer(player, true) -- Замораживаем игрока!
...
selectUpgradeEvent.OnServerEvent:Connect(function(player, choiceIndex)
	...
	if session and session.isPaused and session.currentOptions then
		...
		if selectedUpgrade then
			print("Игрок выбрал: " .. selectedUpgrade.name)
			...
			-- СНИМАЕМ ЗАМОРОЗКУ
			toggleZombies(false) 
			togglePlayer(player, false) -- Игрок снова может бегать!
		end
	end
end)
...
local function startPlayerSession(player, weaponChoice)
	...
	task.spawn(function()
		while activeSessions[player.UserId] and activeSessions[player.UserId].isPlaying do
			...
			local remaining = GAME_TIME - elapsed
			...
			if player.Character and player.Character:FindFirstChild("HumanoidRootPart") and not isDead then
				...
				if exactTime - session.lastAttackTime >= session.stats.cooldown then
					...
					if session.weapon == "Fireball" then
						...
						-- == МЕЧ (С ЗВУКОМ УДАРА) ==
						-- == МЕЧ (ОДИН ОБОРОТ ВОКРУГ) ==
						-- == МЕЧ (ИСПРАВЛЕННЫЙ ТАЙМИНГ ЗВУКА) ==
					elseif session.weapon == "Sword" then
						...
						if handle then
							...
							if radius > 5 then
								local scaleMult = 1 + ((radius - 5) * 0.1)
								newSword:ScaleTo(scaleMult)
							end
							...
							weld.Part0 = root 
							weld.Part1 = handle
							weld.C0 = CFrame.new(radius, 0, 0) * CFrame.Angles(0, math.rad(90), math.rad(45))
							weld.Parent = handle
							...
							task.spawn(function()
								...
								while os.clock() - startTime < duration do
									if not newSword or not newSword.Parent then break end
									local alpha = (os.clock() - startTime) / duration 
									local angle = alpha * (math.pi * 2) 
									weld.C0 = CFrame.Angles(0, angle, 0) * CFrame.new(radius, 1, 0) * CFrame.Angles(0, math.rad(90), math.rad(45))
									task.wait() 
								end

								-- === ВАЖНОЕ ИСПРАВЛЕНИЕ ТУТ ===
								if newSword and newSword.Parent then
									-- Вместо удаления — прячем меч, чтобы звук доиграл
									for _, p in pairs(newSword:GetDescendants()) do
										if p:IsA("BasePart") then
											p.Transparency = 1
											p.CanTouch = false -- Отключаем касания, чтобы не бить во время "исчезновения"
										elseif p:IsA("Trail") or p:IsA("ParticleEmitter") then
											p.Enabled = false
										end
									end

									-- Удаляем окончательно через 1 секунду
									game:GetService("Debris"):AddItem(newSword, 1)
								end
							end)
			...
			if currentTime - session.lastEnemySpawnTime >= SPAWN_RATE then
				...
				if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
					local root = player.Character.HumanoidRootPart
					local offset = Vector3.new(math.random(-50, 50), 5, math.random(-50, 50))
					...
					if hum then
						hum.Died:Connect(function()
							...
							if math.random(1, 100) <= 5 and appleTemplate then
								...
								game:GetService("Debris"):AddItem(drop, 180)
							end
							task.wait(1)
							newEnemy:Destroy()
						end)
...
Players.PlayerAdded:Connect(function(player)
	...
	player.CharacterAdded:Connect(function(character)
		...
		if activeSessions[player.UserId] then activeSessions[player.UserId].isPlaying = false end
	end)
end)
